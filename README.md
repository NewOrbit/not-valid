...
const mustContainA = (value: string) => {
    return value.indexOf("A") === -1 ? "Value must contain the letter 'a'" : null;
};
...
const lengthWithinBounds = (min: number, max: number) => {
    return (value: string) => {
        const invalidLength = value.length < min || value.length > max;

        return invalidLength ? `Value must have length between ${min} and ${max}` : null;
    };
};
